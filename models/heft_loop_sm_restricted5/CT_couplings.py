...
######################################
# R2 couplings for the MHEFT insertions
######################################


R2GC_1297_467 = Coupling(name = 'R2GC_1297_467',
                        # below, multiplied CT2, CB2 by 2/3 to conform with experimental definitions
                        value = '2*CT2*4.0*(-yt**2/2.0)*(1.0/2.0)*((complex(0,1)*G**2)/(8.0*cmath.pi**2)) + (2*CT1)*4.0*(-yt**2/2.0)*(1.0/2.0)*((complex(0,1)*G**2)/(8.0*cmath.pi**2))',
                         order = {'MHEFT':1,'QCD':2,'QED':0})

R2_GGHH_DoubleC1 = Coupling(name = 'R2_GGHH_DoubleC1',
                        value = '(CT1**2)*4.0*(-yt**2/2.0)*(1.0/2.0)*((complex(0,1)*G**2)/(8.0*cmath.pi**2))',
                        order = {'MHEFT':2,'QCD':2,'QED':0})

R2_GGH_C1 = Coupling(name = 'R2_GGH_C1',
                value = 'CT1*4.0*(-((complex(0,1)*yt)/cmath.sqrt(2)))*(1.0/2.0)*(G**2/(8.0*cmath.pi**2))*MT',
                order = {'MHEFT':1,'QCD':2,'QED':0})
