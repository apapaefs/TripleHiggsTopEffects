...
export POINTS=${POINTS:-$repository_root/scans/ct3.14tev-mu75-shape-replacements.csv}
export OUTPUT_DIR=${OUTPUT_DIR:-$repository_root/artifacts/lhe/14tev-ct3-mu75-shapes}
export WORK_DIR=${WORK_DIR:-$repository_root/.work/14tev-ct3-mu75-shapes}
export LOG_DIR=${LOG_DIR:-$repository_root/logs/14tev-ct3-mu75-shapes}
export FIGURE_OUTPUT=${FIGURE_OUTPUT:-$repository_root/artifacts/figures/14tev-ct3-mu75-benchmarks}
...
# Odysseus has 384 logical CPUs.  Run both benchmarks concurrently with
# exactly 192 slots assigned to each point.
export TOTAL_CORES=${TOTAL_CORES:-384}
export CORES_PER_POINT=${CORES_PER_POINT:-192}
export SEED_START=${SEED_START:-65401}
...
python3 scripts/plot_ct3_shapes.py \
  ...
  --sample "$manifest" 2.1 16 -2.3 \
  --sample "$manifest" 6.0 64 -0.48 \
...
if [[ "${UPDATE_DRAFT:-1}" == 1 ]]; then
  install -m 0644 \
    ...
    "$repository_root/HHH_YR5/14tev-ct3-mu75-benchmarks-m3h.pdf"
  install -m 0644 \
    "$FIGURE_OUTPUT-sum-pth.pdf" \
    "$repository_root/HHH_YR5/14tev-ct3-mu75-benchmarks-sum-pth.pdf"
fi
