...
\end{center}
\vspace{-4mm}
\caption{\label{fig:ct3benchmarks} \DIFaddFL{Normalised $m_{3h}$ and $\sum p_{T,h}$ distributions at the HL-LHC for the SM and the two $\mu_{3h}\simeq75$ benchmarks $(\kappa_3,\kappa_4,\kappa_{3t})=(2.1,16,-2.3)$ and $(6.0,64,-0.48)$. The distributions use the same $40\,\mathrm{GeV}$ binning as Figures~\ref{fig:distributions} and~\ref{fig:distributions2}, with wider displayed ranges to retain the hard benchmark's high-energy tail. We fix $\kappa_t=1$ and $\kappa_{2t}=0$.}}
\end{figure}

\DIFadd{The two mixed-coupling benchmarks in~Figure~\ref{fig:ct3benchmarks} provide a particularly clear example of an enhanced but nearly degenerate inclusive rate. Their $100\,000$-event samples give $\mu_{3h}=74.97$ and $75.09$, respectively, so their inclusive cross sections differ by only $0.16\%$. Nevertheless, their normalised distributions differ strongly both from the SM and from one another. The $(2.1,16,-2.3)$ benchmark shifts weight towards larger invariant masses and transverse momenta, whereas the $(6.0,64,-0.48)$ benchmark enhances the near-threshold and low-$p_T$ regions. The samples use the same PDFs and scale choice as the inclusive-rate parametrisation.
}
