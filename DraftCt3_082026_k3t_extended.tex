...
Triple-Higgs production at hadron colliders has been the subject of extensive theoretical investigation over the past two decades~\cite{Plehn:2005nk,Binoth:2006ym,Maltoni:2014eza,Papaefstathiou:2015paa,Chen:2015gva,Fuks:2015hna,deFlorian:2016sit,Kilian:2017nio,Fuks:2017zkg,Bizon:2018syu,deFlorian:2019app,Stylianou:2023xit,Papaefstathiou:2023uum,Abouabid:2024gms,Dong:2025lkm,Haisch:2025pql,Campbell:2025llb,Fuks:2025gjv,Panizzi:2025sya,Ding:2026qto}. Within the SM, the gluon-fusion process $gg \to 3h$ is dominated by the one-loop topologies shown in the upper and middle rows of Figure~\ref{fig:diagrams}. In the figure, green (blue) boxes denote insertions of the Higgs trilinear (quartic) self-coupling, while orange circles represent the top-quark Yukawa coupling. The two diagrams in the lower row illustrate representative BSM contributions involving a contact interaction between two top quarks and two Higgs bosons. In the SM, bottom-quark loops modify the $gg \to 3h$ production cross section by only about $0.2\%$ at LHC energies~\cite{Campbell:2025llb}; they are nevertheless retained in the numerical calculations below.
...
We next extend the inclusive-rate parametrisation to the two-top--three-Higgs interaction in~Eq.~(\ref{eq:Lkappa}). We generate loop-induced LO parton-level samples with \texttt{MadGraph5\_aMC@NLO}~3.5.16~\cite{Alwall:2014hca} and a restricted UFO implementation of the $\kappa_{3t}$ contact term. We again use $m_h=125\,\mathrm{GeV}$, $m_t=173\,\mathrm{GeV}$, the central member of \texttt{PDF4LHC21\_40} (LHAPDF ID~93100), and $\mu_R=\mu_F=m_{3h}/2$. In this scan $\kappa_t=1$ and $\kappa_{2t}=0$ are fixed. The generated MadGraph amplitudes include bottom-quark loops with $m_b=4.70\,\mathrm{GeV}$; the $0.07\,\mathrm{GeV}$ difference from the MCFM input has a negligible effect at the precision of the fit.

Following the form of Eq.~(\ref{eq:BSMLO}), we write the extended cross section as
...
The two mixed-coupling benchmarks in~Figure~\ref{fig:ct3benchmarks} provide more dramatic examples. Their inclusive cross sections are $43.571\,\mathrm{ab}$ and $41.744\,\mathrm{ab}$, respectively, compared with $43.391\,\mathrm{ab}$ for the SM sample, corresponding to rate ratios of $1.004$ and $0.962$. Nevertheless, the normalised bin-by-bin ratios exhibit order-one changes. For $(2.10,17,-0.20)$ they range from approximately $0.07$ to $4.6$ in $m_{3h}$ and from $0.17$ to $3.7$ in $\sum p_{T,h}$; for $(1.9,0,0.40)$ the corresponding ranges are approximately $0.07$--$7.2$ and $0.10$--$12.7$. These ranges retain only bins containing at least $0.2\%$ of the normalised SM sample, reducing sensitivity to statistical fluctuations in the far tails. The pronounced dip of the first benchmark around $m_{3h}\simeq750$--$800\,\mathrm{GeV}$ is generated by destructive interference between the standard loop topologies and the amplitudes involving the modified self-couplings and $\kappa_{3t}$ contact interaction; compensating enhancements at lower invariant mass and in parts of the tail keep the inclusive rate SM-like.

The dedicated distribution campaign instead uses the central member of \nolinkurl{NNPDF40_lo_as_01180} (LHAPDF ID~331900). MadGraph dynamical-scale choice~3 sets
\beq
\mu_R=\mu_F=\frac{H_T}{2}\,,\qquad H_T=\sum_h\sqrt{m_h^2+p_{T,h}^2}\,.
\eeq
The quoted rates use their common $43.391\,\mathrm{ab}$ SM reference rather than Eq.~(\ref{eq:SMLO3t}), which employs the \texttt{PDF4LHC21\_40} and $m_{3h}/2$ setup. The rate ratios and normalised shape comparisons in Figure~\ref{fig:ct3benchmarks} are internally consistent.
